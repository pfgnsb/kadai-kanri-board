...
この資料は論理データモデルの図です。いまのアプリが3つの表に分けて保存している、という意味ではありません。  
いまの保存は localStorage の JSON 1件です。表の項目・型・制約の正は「データベース設計」とします。
...
| 種類 | 内容 |
|---|---|
| いまの保存 | JSON 1件。ボードの中にリストの配列、リストの中にカードの配列 |
| この ER 図 | ドメインと、将来 SQLite を足すときのための論理モデル |
| 実装状況 | SQLite は未実装。表への分割も未実装 |

```mermaid
flowchart TD
  domain[論理モデル：boards / lists / cards] --> json[いまの保存：入れ子の JSON 1件]
  domain -.-> sqlite[将来案：SQLite の3表]
```
...
| カードの移動 | 所属の付け替え | 「左へ」「右へ」はカードの所属リストを隣へ変える。カード自体は増えない |
