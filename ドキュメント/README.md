...
| 画面 | `frontend/` | Vite で起動し、ボードの表示、リスト・カードの追加と削除、カードの更新、つまんで置く移動ができる |
| API | `backend/` | `GET /api/health`、`GET /api/board`、リストとカードの追加、`PUT /api/cards/{cardId}`、`POST /api/cards/{cardId}/move`、`DELETE /api/lists/{listId}`、`DELETE /api/cards/{cardId}` |
...
配置とボタンの役割は画面設計書です。先に形をつかむときは画面見取り図を見ます。HTMLモックアップ作成計画は、既存の `index.html` を認識合わせに使う約束です。`frontend/` の React 画面は、その設計のうちボードの表示、リスト・カードの追加と削除、カードの更新、つまんで置く移動まで進んでいます。
