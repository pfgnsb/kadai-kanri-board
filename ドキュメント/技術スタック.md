...
| `backend/` | Spring Boot の API。JDBC で PostgreSQL につなぐ。`GET /api/health` と `GET /api/board` がある |
| `frontend/` | Vite の React。起動時に `GET /api/board` の結果を表示する |
...
5. `frontend` で Vite の開発サーバを起動し、`http://localhost:5173` でボードの読み取り結果を確認する
...
画面の部品と動きの正は「画面設計書」とする。いまの `frontend/` は、ボード取得 API の結果を表示するところまでである。認識合わせ用の HTML モックはフォルダ直下に残す。
...
| 形式 | REST。JSON で返す |
| いま動くもの | `GET /api/health` → `{"status":"ok","database":"up"}`。`GET /api/board` がボードを返す。接続時に boards / lists / cards を作る |
| 画面とのつなぎ | 開発中は CORS を許可し、Vite の画面から `fetch` で `GET /api/board` を呼ぶ |
...
いま動くのは起動確認の `/api/health` と、ボード読み取りの `/api/board` である。リストとカードの更新は、これからエンドポイントを足す。
