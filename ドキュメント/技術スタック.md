...
| `backend/` | Spring Boot の API。JDBC で PostgreSQL につなぐ。ボードの読み取りと、リスト・カードの追加がある |
| `frontend/` | Vite の React。ボードを表示し、リストとカードの追加・削除、カードの更新、つまんで置く移動ができる |
...
環境を実装したときに固定した版である。画面の版は `frontend/package-lock.json`、API の版は `backend/pom.xml` と Maven Wrapper、PostgreSQL の版は `docker-compose.yml` を正とする。

| 技術 | 版 | 置き場所 |
|---|---|---|
| React | 19.3.0 | `frontend/package-lock.json`（宣言は `react` `^19.1.1`） |
| react-dom | 19.3.0 | 同上（宣言は `react-dom` `^19.1.1`） |
| Vite | 7.3.6 | 同上（宣言は `vite` `^7.1.5`） |
| @vitejs/plugin-react | 5.2.0 | 同上（宣言は `@vitejs/plugin-react` `^5.0.2`） |
| Node.js | 20.19.0 以上、または 22.12.0 以上 | Vite 7 が要求する実行環境 |
| Java | 21 | `backend/pom.xml` の `java.version`。JDK の例は Microsoft Build of OpenJDK 21.0.12 |
| Spring Boot | 4.1.1 | `backend/pom.xml` の parent |
| Maven | 3.9.16 | Maven Wrapper 3.3.4 が取得する配布物 |
| PostgreSQL | 16 | `docker-compose.yml` の `postgres:16-alpine` |

`package.json` の `^` は、その範囲で入る版を許す書き方である。いま入っている版は lock ファイルの行を正とする。

---

## 3. 提出物と動かし方

提出物はフォルダ一式とする。本実装を動かすには JDK 21、Docker Desktop、Node.js（20.19.0 以上、または 22.12.0 以上）が必要である。
...
5. `frontend` で Vite の開発サーバを起動し、`http://localhost:5173` でボードの表示と、リスト・カードの追加を確認する
...
| 採用するもの | 層 | 理由 |
|---|---|---|
| React 19.3.0 | 画面 | 指定。ボード・リスト・カードをコンポーネントに分けて組む |
| Vite 7.3.6 | 画面 | Next.js を使わない React の標準的な開発サーバ。JSX をそのまま扱える。プラグインは `@vitejs/plugin-react` 5.2.0 |
...
| PostgreSQL JDBC | API | PostgreSQL へ SQL を出す。版は Spring Boot 4.1.1 が管理する |
| PostgreSQL 16 | 保存 | 指定。リストとカードを表に分けて残す。イメージは `postgres:16-alpine` |
...
| ランタイム | API は JDK 21。画面の開発サーバは Node.js 20.19.0 以上、または 22.12.0 以上 |
| データベース実行 | Docker Desktop。PostgreSQL 16（`postgres:16-alpine`）をコンテナで動かす |
...
| 状態 | ボードデータは API から取得し、コンポーネントの state で持つ。追加後はその結果を state に足す |
...
画面の部品と動きの正は「画面設計書」とする。いまの `frontend/` は、ボードの表示、リスト・カードの追加と削除、カードの更新、つまんで置く移動までである。認識合わせ用の HTML モックはフォルダ直下に残す。
...
| ビルド | Maven Wrapper 3.3.4（Maven 3.9.16、`backend/mvnw.cmd`） |
| 形式 | REST。JSON で返す |
| いま動くもの | 起動確認、ボード読み取り、リストとカードの追加、`PUT /api/cards/{cardId}`、`POST /api/cards/{cardId}/move`、`DELETE /api/lists/{listId}`、`DELETE /api/cards/{cardId}`。接続時に boards / lists / cards の表を作る |
| 画面とのつなぎ | 開発中は CORS で GET、POST、PUT、DELETE を許可し、Vite の画面から `fetch` で呼ぶ |
...
いま動くのは起動確認、ボード読み取り、リストとカードの追加、カード更新の `PUT /api/cards/{cardId}`、移動先を指定する `POST /api/cards/{cardId}/move`、リスト削除の `DELETE /api/lists/{listId}`、カード削除の `DELETE /api/cards/{cardId}` である。

---

## 8. 保存まわりの技術（PostgreSQL）
...
| ドラッグ用のライブラリ | カードの移動は、ブラウザ標準のポインタ操作で行う |
...
| 2.4 | 2026年9月25日 | 実装で固定した React、Vite、Node.js、Java、Spring Boot、Maven、PostgreSQL の版を追記した |
| 2.5 | 2026年9月26日 | リストとカードの追加 API と、画面の追加操作を実装した |
| 2.6 | 2026年9月26日 | カードの更新と、左右のリストへの移動を実装した |
| 2.7 | 2026年9月26日 | カードの移動を、標準のポインタ操作でつまんで置くようにした |
| 2.8 | 2026年9月26日 | リストとカードの削除を実装した |
