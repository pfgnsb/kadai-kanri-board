...
| `frontend/` | Vite の React。ボードを表示し、リストとカードの追加、カードの更新、つまんで置く移動ができる |
...
画面の部品と動きの正は「画面設計書」とする。いまの `frontend/` は、ボードの表示、リスト・カードの追加、カードの更新、つまんで置く移動までである。認識合わせ用の HTML モックはフォルダ直下に残す。削除ボタンはまだ出さない。
...
| 形式 | REST。JSON で返す |
| いま動くもの | 起動確認、ボード読み取り、リストとカードの追加、`PUT /api/cards/{cardId}`、`POST /api/cards/{cardId}/move`。接続時に boards / lists / cards の表を作る |
| 画面とのつなぎ | 開発中は CORS で GET、POST、PUT を許可し、Vite の画面から `fetch` で呼ぶ |
...
いま動くのは起動確認、ボード読み取り、リストとカードの追加、カード更新の `PUT /api/cards/{cardId}`、移動先を指定する `POST /api/cards/{cardId}/move` である。削除は、これからエンドポイントを足す。
