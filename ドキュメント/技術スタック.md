...
| `frontend/` | Vite の React。ボードを表示し、リストとカードの追加、カードの更新と左右移動ができる |
...
画面の部品と動きの正は「画面設計書」とする。いまの `frontend/` は、ボードの表示、リスト・カードの追加、カードの更新と左右移動までである。認識合わせ用の HTML モックはフォルダ直下に残す。削除ボタンはまだ出さない。
...
いま動くのは起動確認、ボード読み取り、リストとカードの追加、カード更新の `PUT /api/cards/{cardId}`、左右移動の `POST /api/cards/{cardId}/move` である。削除は、これからエンドポイントを足す。
