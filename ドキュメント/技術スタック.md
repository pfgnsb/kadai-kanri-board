...
| `frontend/` | Vite の React。ボードを表示し、リストとカードを追加できる |
...
画面の部品と動きの正は「画面設計書」とする。いまの `frontend/` は、ボードの表示と、リスト・カードの追加までである。認識合わせ用の HTML モックはフォルダ直下に残す。
...
| 形式 | REST。JSON で返す |
| いま動くもの | `GET /api/health`、`GET /api/board`、`POST /api/lists`、`POST /api/lists/{listId}/cards`。接続時に boards / lists / cards の表を作る |
| 画面とのつなぎ | 開発中は CORS で GET と POST を許可し、Vite の画面から `fetch` で呼ぶ |
...
いま動くのは起動確認の `/api/health`、ボード読み取りの `/api/board`、リスト追加の `POST /api/lists`、カード追加の `POST /api/lists/{listId}/cards` である。変更・削除・移動は、これからエンドポイントを足す。
